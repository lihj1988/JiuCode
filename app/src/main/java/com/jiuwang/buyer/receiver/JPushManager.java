...
//        String jsonJpushMessage = jPushMessage.getTags().toString();
//        jsonJpushMessage = jsonJpushMessage.substring(1, jsonJpushMessage.length() - 1);
//        init(context);
...
//    }
//
//    private void sortMethod(String[] arrayNew) {
//        int start = 0;
//        int end = arrayNew.length - 1;
//        String[] a = sort(arrayNew, start, end);
//        Log.i(TAG, a + "");
//    }
//
...
//        while (end > start) {
//            //从后往前比较
//            while (end > start && a[end].compareTo(key) >= 0)  //如果没有比关键值小的，比较下一个，直到有比关键值小的交换位置，然后又从前往后比较
//                end--;
...
//            }
//            //从前往后比较
//            while (end > start && a[start].compareTo(key) <= 0)//如果没有比关键值大的，比较下一个，直到有比关键值大的交换位置
//                start++;
